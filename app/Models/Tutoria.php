<?php
    ...
    class Tutoria{
        ...
        public function registarTutoria($id_disciplina, $id_docente, $hora_inicio, $hora_termino, $data_realizacao, $descricao) {
            ...
            $sqlCriar = "INSERT INTO `tutoria` (`id_disciplina`, `id_docente`, `hora_inicio`, `hora_termino`, `data_registo`, `data_realizacao`, `descricao`)
                         VALUES (?, ?, ?, ?, NOW(), ?, ?)";
            $stmt = $connection->prepare($sqlCriar);
            $stmt->bind_param("iissss", $id_disciplina, $id_docente, $hora_inicio, $hora_termino, $data_realizacao, $descricao);
    
            if ($stmt->execute()) {
                return $connection->insert_id; 
            } else {
                echo "Erro ao registar Tutoria: " . $stmt->error;
                return null;
            }
        }
